...
const deploymentRef = process.env.VERCEL_GIT_COMMIT_REF ?? "vercel-prod";
const deploymentSha = process.env.VERCEL_GIT_COMMIT_SHA?.slice(0, 7) ?? process.env.VERCEL_DEPLOYMENT_ID?.slice(-7) ?? "local";
        ...
        <main className="terminal-content">
          ...
          <section className="section" id="about">
            ...
            <div className="neo-panel">
              ...
              <div className="identity"><div className="identity-title">shivam@portfolio <span>----------------</span></div><dl>
                <div><dt>OS</dt><dd>Human v21.0 (engineer build)</dd></div><div><dt>Host</dt><dd>Remote, India</dd></div>
                <div><dt>Role</dt><dd>Full-stack developer</dd></div><div><dt>Uptime</dt><dd>3+ years, many shipped</dd></div>
                <div><dt>Stack</dt><dd>Python, JS, React, Next</dd></div>
          ...
          <section className="section" id="work">
            ...
            <div className="project-grid">{projects.map((project) => <article className="project-card" key={project.title}>
              ...
              <p>{project.description}</p><div className="tags">{project.techStack.map((tech) => <span key={tech}>{tech}</span>)}{project.tag ? <span className="amber-tag">{project.tag}</span> : <span className="amber-tag">open source</span>}</div>
              <div className="project-links">{project.link && <a href={project.link} target="_blank" rel="noreferrer">-&gt; live demo</a>}{project.links?.map((link) => <a key={link.href} href={link.href} target="_blank" rel="noreferrer">-&gt; {link.label.toLowerCase()}</a>)}{project.github && <a href={project.github} target="_blank" rel="noreferrer">-&gt; source</a>}</div>
            </article>)}</div>
          </section>
          ...
          <section className="section" id="stack"><Prompt>cat stack.txt | sort -r</Prompt><div className="section-kicker">&gt; stack</div><h2>The tools I reach for <span>{"// always learning, always shipping"}</span></h2>
            <div className="skills-panel toolbox-panel">{[...skills].sort((a, b) => b.name.localeCompare(a.name)).map((skill, index) => <div className="tool-item" key={skill.name}><span className="tool-index">[{String(index + 1).padStart(2, "0")}]</span><img src={skill.icon} alt="" /><span>{skill.name}</span><b>-&gt;</b></div>)}</div>
          </section>
          ...
          <footer><div>$ echo &quot;(c) 2026 Shivam, built in the terminal, shipped fast&quot; <i className="cursor-block small" /></div><span>last deploy: {deploymentRef}@{deploymentSha}, uptime 99.98%</span></footer>
        </main>
      </div>
    </>
  );
